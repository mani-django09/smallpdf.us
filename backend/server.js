  ...
  // Middleware
  app.use(cors({
  origin: ['http://localhost:3000', 'http://localhost:3006', 'http://localhost:3002', 'https://smallpdf.us'],
  credentials: true,
  ...
  // COMPLETE FIXED fileValidation MIDDLEWARE
  // Replace the entire fileValidation function in server.js (starting around line 159)
  ...
  const fileValidation = (req, res, next) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        // Verify file header matches extension - FIXED FOR JPEG AND WEBP
  ...
  // FIXED: Allow .jpg/.jpeg interchangeably, and skip validation for WEBP
  if (detectedType) {
    const isJpegFile = ext === ".jpg" || ext === ".jpeg"
    const isJpegDetected = detectedType === "jpg" || detectedType === "jpeg"
    const isWebpFile = ext === ".webp"
    
    // Allow if both are JPEG variants
    if (isJpegFile && isJpegDetected) {
      // This is fine - .jpg and .jpeg are the same
    }
    // Skip strict validation for WEBP files (complex format)
    else if (isWebpFile) {
      // WEBP validation is complex, just ensure it's not executable
      // Allow it to pass
    }
  ...
  // PDF to JPG
  // PDF to JPG - FIXED VERSION
// Replace the existing /api/pdf-to-jpg endpoint in server.js with this code
...
app.post("/api/pdf-to-jpg", upload.single("file"), fileValidation, async (req, res) => {
  ...
  // ENHANCED PDF TO WORD ENDPOINT WITH LAYOUT PRESERVATION
  // Add this to your server.js file, replacing the existing /api/pdf-to-word endpoint
  ...
  app.post("/api/pdf-to-word", upload.array("files", 10), fileValidation, async (req, res) => {
    try {
      ...
      for (let i = 0; i < req.files.length; i++) {
        ...
        try {
          console.log(`Converting: ${file.originalname} to Word format with layout preservation...`)

          // Enhanced LibreOffice command with better layout preservation
          // The infilter forces PDF import which preserves layout better
          const command = `"${libreOfficePath}" --headless --infilter="writer_pdf_import" --convert-to "docx:MS Word 2007 XML" --outdir "${convertedDir}" "${file.path}"`

          console.log(`Executing: ${command}`)

          await execPromise(command, {
            timeout: 180000, // 3 minutes timeout for complex PDFs
          ...
          const possiblePaths = [
            expectedOutputPath,
            path.join(convertedDir, path.basename(file.path).replace(/\.pdf$/i, ".docx")),
            file.path.replace(/\.pdf$/i, ".docx"),
            path.join(path.dirname(file.path), path.basename(file.path).replace(/\.pdf$/i, ".docx"))
          ]
          ...
          if (fs.existsSync(expectedOutputPath)) {
            ...
            try {
              ...
              console.log("Could not read PDF page count:", err.message)
            }
            ...
            console.log(`   Pages: ${pageCount}, Size: ${(stats.size / 1024).toFixed(1)} KB`)
          } else {
            console.error(`DOCX file not found at expected path: ${expectedOutputPath}`)
            console.error(`Checked paths:`, possiblePaths)
            throw new Error("DOCX file not created. Make sure LibreOffice 7.0+ is installed with PDF import support.")
          }
      ...
      if (convertedFiles.length === 0) {
        ...
        logActivity({
          ...
          tool: "pdf-to-word",
          error: "No files converted - LibreOffice may not be installed or PDFs may be corrupted/password-protected",
          ip: req.ip || req.headers["x-forwarded-for"],
          status: "error",
        })
      ...
      res.json({
        success: true,
        message: `${convertedFiles.length} PDF(s) converted to Word successfully with layout preservation`,
        jobId: jobId,
        fileCount: convertedFiles.length,
        files: convertedFiles,
      })
  ...
  // Enhanced download endpoint for PDF to Word converted files
