...
export default function DownloadPdfToPpt() {
  ...
  useEffect(() => {
    setMounted(true)
    const result = sessionStorage.getItem("pdfToPptConvertResult")
    if (result) {
      try {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const downloadUrl = `${apiUrl}/api/download-ppt/${convertResult.jobId}`
      ...
      let filename = convertResult.files?.[0]?.filename || "presentation.pptx"
      ...
      // Show error inline — no alert(), uses i18n, doesn't block UI
      setDownloadError(t('pdfToPpt.downloadFailed') || `Failed to download file: ${error.message}`)
    } finally {
  ...
  const handleConvertAnother = () => {
    sessionStorage.removeItem("uploadedPdfFilesForPpt")
    sessionStorage.removeItem("pdfToPptConvertResult")
    router.push("/pdf-to-ppt")
  }
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: t('pdfToPpt.shareTitle') || "My Converted PowerPoint",
          text: t('pdfToPpt.shareText') || "Check out my converted PDF to PowerPoint presentation",
          url: window.location.href,
        })
      } catch (err) {
        // Share cancelled or failed — not an error worth surfacing to the user
      }
    } else {
      handleCopyLink()
    }
  ...
  if (!convertResult && !jobId) {
    return (
      <Layout>
        <SEOHead title={t('pdfToPpt.downloadPageTitle') || "Download PowerPoint | SmallPDF.us"} noIndex={true} />
        <div className="min-h-screen flex items-center justify-center bg-red-50">
          <div className="text-center">
            ...
            <p className="text-slate-600 font-medium text-sm">{t('pdfToPpt.loadingPptx')}</p>
          </div>
        </div>
      </Layout>
    )
  ...
  return (
    <Layout title={t('pdfToPpt.downloadPageTitle')} description={t('pdfToPpt.downloadPageDescription')}>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className={`${mounted ? "animate-fade-up-delay-1" : "opacity-0"}`}>
                <h1 className="font-display text-xl sm:text-2xl font-bold text-white tracking-tight">
                  {t('pdfToPpt.downloadSuccessTitle')}
                </h1>
                <p className="font-body text-red-100 text-sm">
                  {fileCount} PDF{fileCount > 1 ? "s" : ""} {t('pdfToPpt.downloadSuccessSubtitle')}
                </p>
              </div>
            </div>
      ...
      {/* Main Content */}
      <div className="bg-red-50 py-8 sm:py-12 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
            ...
            <div className="lg:col-span-8 space-y-6">
              <div className="bg-white rounded-2xl border border-red-200 shadow-lg overflow-hidden">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className="font-display text-lg sm:text-xl font-semibold text-white truncate">
                        {files.length === 1 ? files[0]?.filename : `${fileCount} PowerPoint Files`}
                      </h2>
                ...
                {files.length > 0 && (
                  <div className="px-6 py-4 border-b border-red-100 bg-red-50/50">
                    <h3 className="font-display text-sm font-semibold text-slate-900 mb-3">{t('pdfToPpt.convertedFilesHeading')}</h3>
                    <div className="space-y-2">
                      {files.map((file, index) => (
                        <div key={index} className="flex items-center gap-3 p-2 bg-white rounded-lg border border-red-100">
                ...
                <div className="p-6 sm:p-8">
                  ...
                  <button
                    ...
                    className="group relative w-full bg-gradient-to-r from-red-500 to-orange-500 text-white py-4 sm:py-5 px-6 rounded-xl font-semibold text-base sm:text-lg transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-3 shadow-lg hover:shadow-xl hover:from-red-600 hover:to-orange-600 overflow-hidden"
                  >
                    <div className="absolute inset-0 btn-shimmer"></div>
                    {downloading ? (
                      <>
                        <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                        <span className="relative">{t('pdfToPpt.preparingDownload')}</span>
                      </>
                    ) : downloadComplete ? (
                      <>
                        <CheckCircle className="w-5 h-5" />
                        <span className="relative">{t('pdfToPpt.downloadedSuccess')}</span>
                      </>
                    ) : (
                      <>
                        <Download className="w-5 h-5 group-hover:animate-bounce" />
                        <span className="relative">
                          {files.length > 1 ? t('pdfToPpt.downloadAllZip') : t('pdfToPpt.downloadPptxFile')}
                        </span>
                  ...
                  <div className="grid grid-cols-3 gap-3 mt-6">
                    <div className="text-center p-4 bg-red-50 rounded-xl border border-red-100">
                      ...
                      <div className="font-body text-xs text-slate-500 mt-1">{fileCount > 1 ? t('pdfToPpt.statsFiles') : t('pdfToPpt.statsFile')}</div>
                    </div>
                    <div className="text-center p-4 bg-red-50 rounded-xl border border-red-100">
                      <div className="font-display text-2xl sm:text-3xl font-bold text-orange-600">PPTX</div>
                      <div className="font-body text-xs text-slate-500 mt-1">{t('pdfToPpt.statsFormat')}</div>
                    </div>
                    <div className="text-center p-4 bg-red-50 rounded-xl border border-red-100">
                      <div className="font-display text-2xl sm:text-3xl font-bold text-green-600">100%</div>
                      <div className="font-body text-xs text-slate-500 mt-1">{t('pdfToPpt.statsEditable')}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-6 pt-6 border-t border-red-100">
                    <button
                      ...
                      className="flex-1 py-3 px-4 rounded-xl font-medium text-sm bg-red-100 hover:bg-red-200 transition-colors flex items-center justify-center gap-2 text-slate-700"
                    >
                      <RefreshCw className="w-4 h-4" />
                      <span>{t('pdfToPpt.convertMore')}</span>
                    </button>
                    <button
                      onClick={handleShare}
                      className="py-3 px-4 rounded-xl font-medium text-sm bg-red-100 hover:bg-red-200 transition-colors flex items-center justify-center gap-2 text-slate-700"
                    >
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { icon: Edit3, title: t('pdfToPpt.dlFeature1Title'), desc: t('pdfToPpt.dlFeature1Desc'), color: "bg-orange-600" },
                  { icon: Layers, title: t('pdfToPpt.dlFeature2Title'), desc: t('pdfToPpt.dlFeature2Desc'), color: "bg-red-600" },
                  { icon: Shield, title: t('pdfToPpt.dlFeature3Title'), desc: t('pdfToPpt.dlFeature3Desc'), color: "bg-slate-600" },
                  { icon: Zap, title: t('pdfToPpt.dlFeature4Title'), desc: t('pdfToPpt.dlFeature4Desc'), color: "bg-amber-500" },
                ].map((item, i) => (
              ...
              <div className="bg-white rounded-2xl border border-red-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-red-100 bg-red-50">
                  <h3 className="font-display text-lg font-semibold text-slate-900">{t('pdfToPpt.exploreMoreTitle')}</h3>
                </div>
                <div className="p-6">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      {
                        name: t('pdfToPpt.moreTool1Name'),
                        desc: t('pdfToPpt.moreTool1Desc'),
                        href: "/ppt-to-pdf",
                        color: "bg-orange-600",
                      },
                      { name: t('pdfToPpt.moreTool2Name'), desc: t('pdfToPpt.moreTool2Desc'), href: "/pdf-to-word", color: "bg-blue-600" },
                      { name: t('pdfToPpt.moreTool3Name'), desc: t('pdfToPpt.moreTool3Desc'), href: "/merge-pdf", color: "bg-purple-600" },
                    ].map((tool) => (
                      <a
                        key={tool.name}
                        href={router.href(tool.href)}
                        className="group p-4 rounded-xl border border-red-200 hover:border-red-300 hover:shadow-lg transition-all duration-300 bg-white"
                      >
                        <div
                        ...
                        <span className="inline-flex items-center text-sm font-medium text-slate-700 group-hover:text-red-600 transition-colors">
                          {t('pdfToPpt.tryNow')}
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-red-200 overflow-hidden">
                <div className="px-5 py-4 border-b border-red-100 bg-red-50">
                  <h3 className="font-display text-base font-semibold text-slate-900">{t('pdfToPpt.conversionSummaryHeading')}</h3>
                </div>
                <div className="p-5 space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="font-body text-sm text-slate-600">{t('pdfToPpt.filesConvertedLabel')}</span>
                  ...
                  <div className="pt-3 border-t border-red-100">
                    <div className="flex justify-between items-center">
                      <span className="font-body text-sm text-slate-600">{t('pdfToPpt.statusLabel')}</span>
                      <span className="inline-flex items-center gap-1.5 text-green-600 font-medium text-sm">
                        <CheckCircle className="w-4 h-4" />
                        {t('pdfToPpt.statusComplete')}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl border border-red-200 p-5">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-display text-sm font-semibold text-slate-900">{t('pdfToPpt.fileExpiryTitle')}</h3>
                    <p className="font-body text-xs text-slate-500">{t('pdfToPpt.fileExpirySubtitle')}</p>
                  </div>
                </div>
                <div className="bg-red-50 rounded-lg p-3">
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-body text-xs text-slate-600">{t('pdfToPpt.timeRemainingLabel')}</span>
              ...
              <div className="bg-white rounded-2xl border border-red-200 p-5">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h3 className="font-display text-sm font-semibold text-slate-900">{t('pdfToPpt.privacyHeading')}</h3>
                </div>
