...
export default function DownloadPngToWebp() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const downloadUrl = `${API_URL}/api/download-webp-converted/${jobId}`
      ...
      alert(t('pngToWebp.errorDownloadFailed') || "Failed to download file. Please try again.")
    } finally {
      ...
      <Layout>
        ...
        <div className="min-h-screen flex items-center justify-center bg-slate-50">
          <div className="text-center">
            ...
            <p className="text-slate-600 font-medium text-sm">{t('pngToWebp.loadingYourFiles')}</p>
          </div>
        </div>
      </Layout>
    )
  ...
  return (
    <Layout title={t('pngToWebp.downloadTitle')} description={t('pngToWebp.downloadDescription')}>
      <SEOHead noIndex={true} />
      <style jsx global>{`
            ...
            <div className="flex items-center gap-4">
              ...
              <div className={`${mounted ? "animate-fade-up-delay-1" : "opacity-0"}`}>
                <h1 className="font-display text-xl sm:text-2xl font-bold text-white tracking-tight">
                  {t('pngToWebp.conversionCompleteTitle')}
                </h1>
                <p className="font-body text-emerald-100 text-sm">
                  {fileCount} {t(fileCount > 1 ? 'pngToWebp.imagesOptimized' : 'pngToWebp.imageOptimized')}
                </p>
              </div>
            </div>
            ...
            <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1.5">
                        ...
                        <span className="text-slate-400 text-sm">
                          {fileCount} {t(fileCount > 1 ? 'pngToWebp.webpImages' : 'pngToWebp.webpImage')}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="p-6 sm:p-8">
                  ...
                  >
                    ...
                    {downloading ? (
                      <>
                        ...
                        <span className="relative">{t('pngToWebp.preparingDownload')}</span>
                      </>
                    ) : downloadComplete ? (
                      <>
                        <CheckCircle className="w-5 h-5" />
                        <span className="relative">{t('pngToWebp.downloadedSuccess')}</span>
                      </>
                    ) : (
                      <>
                        <Download className="w-5 h-5 group-hover:animate-bounce" />
                        <span className="relative">{t('pngToWebp.downloadWebp')} {fileCount > 1 ? t('pngToWebp.files') : t('pngToWebp.image')}</span>
                  ...
                  <div className="grid grid-cols-3 gap-3 mt-6">
                    <div className="text-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                      ...
                      <div className="font-body text-xs text-slate-500 mt-1">{t('pngToWebp.images')}</div>
                    </div>
                    <div className="text-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <div className="font-display text-2xl sm:text-3xl font-bold text-emerald-600">~35%</div>
                      <div className="font-body text-xs text-slate-500 mt-1">{t('pngToWebp.smaller')}</div>
                    </div>
                    <div className="text-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <div className="font-display text-2xl sm:text-3xl font-bold text-slate-900">WEBP</div>
                      <div className="font-body text-xs text-slate-500 mt-1">{t('pngToWebp.format')}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-6 pt-6 border-t border-slate-100">
                    ...
                    >
                      ...
                      <span>{t('pngToWebp.convertMore')}</span>
                    </button>
                    <button
                      onClick={handleShare}
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  {
                    icon: Sparkles,
                    title: t('pngToWebp.superiorCompression'),
                    desc: t('pngToWebp.smallerThanPng'),
                    color: "bg-emerald-600",
                  },
                  { icon: Zap, title: t('pngToWebp.fastProcessing'), desc: t('pngToWebp.convertedInSeconds'), color: "bg-amber-500" },
                  { icon: Shield, title: t('pngToWebp.secureAndPrivate'), desc: t('pngToWebp.autoDeletedInHour'), color: "bg-slate-600" },
                  { icon: FileImage, title: t('pngToWebp.modernFormat'), desc: t('pngToWebp.browserSupport'), color: "bg-slate-800" },
                ].map((item, i) => (
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50">
                  <h3 className="font-display text-lg font-semibold text-slate-900">{t('pngToWebp.exploreMoreTools')}</h3>
                </div>
                <div className="p-6">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      {
                        name: t('pngToWebp.tool1Name'),
                        desc: t('pngToWebp.tool1Desc'),
                        href: "/webp-to-png",
                        color: "bg-teal-600",
                      },
                      { name: t('pngToWebp.tool2Name'), desc: t('pngToWebp.tool2Desc'), href: "/compress-pdf", color: "bg-rose-600" },
                      { name: t('pngToWebp.tool3Name'), desc: t('pngToWebp.tool3Desc'), href: "/merge-pdf", color: "bg-slate-600" },
                    ].map((tool) => (
                      <a
                        key={tool.name}
                        href={tool.href}
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-100 bg-slate-50">
                  <h3 className="font-display text-base font-semibold text-slate-900">{t('pngToWebp.conversionSummary')}</h3>
                </div>
                <div className="p-5 space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="font-body text-sm text-slate-600">{t('pngToWebp.imagesOptimizedLabel')}</span>
                    <span className="font-display font-semibold text-slate-900">{fileCount}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="font-body text-sm text-slate-600">{t('pngToWebp.outputFormat')}</span>
                    <span className="font-display font-semibold text-emerald-600">WEBP</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="font-body text-sm text-slate-600">{t('pngToWebp.compression')}</span>
                    <span className="font-display font-semibold text-slate-900">{t('pngToWebp.lossless')}</span>
                  </div>
                  <div className="pt-3 border-t border-slate-100">
                    <div className="flex justify-between items-center">
                      <span className="font-body text-sm text-slate-600">{t('pngToWebp.status')}</span>
                      <span className="inline-flex items-center gap-1.5 text-emerald-600 font-medium text-sm">
                        <CheckCircle className="w-4 h-4" />
                        {t('pngToWebp.complete')}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-5">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-display text-sm font-semibold text-slate-900">{t('pngToWebp.fileExpiry')}</h3>
                    <p className="font-body text-xs text-slate-500">{t('pngToWebp.autoDeleteForSecurity')}</p>
                  </div>
                </div>
                <div className="bg-slate-50 rounded-lg p-3">
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-body text-xs text-slate-600">{t('pngToWebp.timeRemaining')}</span>
